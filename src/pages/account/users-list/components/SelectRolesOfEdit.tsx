...
export const SelectRolesOfEdit = (props) => {
  ...
  useEffect(() => {
    ...
    const temp1 =  currentSelectedRoleNames && currentSelectedRoleNames.map(each=>{
      console.log("roleRenderArray,each",each);
      const temp1 = each.map(each1=>{
        if(each1=='TenantAdmin') return 'Admin'
        else return each1
      })
      return temp1
    })
    console.log("roleRenderArray,temp",temp1);

    setNewSelectedRoleNames(temp1)
  }, [currentSelectedRoleNames]);
        ...
        currentSelectedBranchIds.map((item, index) => {
          return (
            <div className={style.selectionRow}>
              ...
              <Select
                disabled={IsSuperAdmin()? false: isSuperAdmin}
                value={newSelectedRoleNames && newSelectedRoleNames[index]}
                mode="multiple"
